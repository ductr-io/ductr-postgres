...
RSpec.describe Ductr::Postgres::PollingTrigger do
  ...
  describe "#callable" do
    ...
    let(:handler_double) { instance_double(Ductr::Postgres::PollingHandler) }

    before do
      allow(Ductr::Postgres::PollingHandler).to receive(:new).and_return(handler_double)
    end

    it "creates a PollingHandler" do
      trigger.send(:callable, method_double)
      expect(Ductr::Postgres::PollingHandler).to have_received(:new).with(method_double, adapter_double)
    end
